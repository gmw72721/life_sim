...
def run(worlds: Dict[str, WorldState], 
    ...
    while any(w.clock.current_time < end_dt for w in worlds.values()):
        ...
        for world_id, world in list(worlds.items()):
            ...
            for actor in list(world.actors.values()):
                if actor.current_ticks_left > 0:
                    ...
                    process_actor_action(actor, world, logger)
...
def process_actor_action(actor, world: WorldState, logger: 'SimulationLogger') -> None:
    """
    ...
    if forced_state:
        ...
        action = actions.lookup_forced(forced_state, actor)
        if action:
            logger.log_action(actor, action, True)
            success = actions.apply_action(action, actor, world)
            if not success:
                logger.log_action(actor, action, False)
        else:
            # No valid action for forced state, stay idle
            actor.state = State.Idle
            actor.substate = "calendar_conflict"
            logger.log_state_change(actor, actor.state, State.Idle)
    else:
        # Normal probability-based action selection
        chosen_action = probability.choose_action(actor, world)
        
        if chosen_action:
            # Handle special time jump action
            if chosen_action == TIME_JUMP:
                handle_time_jump(actor, world, logger)
            else:
                # Apply normal action
                logger.log_action(actor, chosen_action, True)
                success = actions.apply_action(chosen_action, actor, world)
                if not success:
                    logger.log_action(actor, chosen_action, False)
                    # Fallback to idle
                    actor.state = State.Idle
...
def handle_time_jump(actor, world: WorldState, logger: 'SimulationLogger') -> None:
    """
    ...
    logger.log_time_jump(actor, world.world_id, f"{world.world_id}_hypothetical_fork")
...
def run_parallel_simulation(world_manager: WorldManager, 
    ...
    for tick in range(num_ticks):
        ...
        if tick % 50 == 0:
            world_manager.prune_inactive_forks()
    ...
    while tick_count < max_ticks:
        ...
        for actor in list(world_state.actors.values()):
            if actor.current_ticks_left > 0:
                ...
                chosen_action = probability.choose_action(actor, world_state)
...
class SimulationLogger:
    ...
    def log_state_change(self, actor, old_state: State, new_state: State) -> None:
        """
